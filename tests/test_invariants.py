...
def test_inv_21_verbatim_quote_check():
    """Verify inv_21 checks that action evidence_quote is grounded in turn text."""
    dialogue = [
        {"pair": 1, "speaker": "user", "text": "Can we build a\nportfolio website with custom layout?", "attachments": [], "ts": "2026-09-18T10:00:00Z"},
        {"pair": 1, "speaker": "ai", "text": "Sure, I recommend Next.js and Tailwind for this project.", "attachments": [], "ts": "2026-09-18T10:00:05Z"},
    ]
    ledger = [
        {"action_id": "U(1,1)", "outcome_id": "o1", "req_id": "r1", "pair_added": 1, "kind": "create"}
    ]
    # Test verbatim and whitespace-collapsed quotes
    state_valid = {
        "actions": [
            {"id": "U(1,1)", "type": "request", "text": "User asks for website", "role": "SHAPER", "evidence_quote": "build a portfolio website"},
            {"id": "A(1,1)", "type": "suggest", "text": "AI suggests stack", "role": "EXECUTOR", "evidence_quote": "Sure ... Next.js and Tailwind"}
        ]
    }
    assert inv.inv_21_provenance(dialogue, ledger, state_valid) == []

    # Test hallucinated quote that is not grounded
    state_invalid = {
        "actions": [
    ...
    assert "not grounded in" in violations[0]
