...
class CachedLLMClient:
    """Caching wrapper around LLMClient.
    Keys on (filesystem-safe-step, sha256(prompt)).
    Stores individual JSON files under <out>/run/cache/.
    """

    def __init__(self, raw_client, out_dir):
        self._client = raw_client
        self.cache_dir = Path(out_dir) / "run" / "cache"
    ...
    def call_json(self, step, model, prompt):
        ...
        # Build filesystem-safe filename
        safe_step = step.replace(":", "_").replace("/", "_").replace(" ", "_")
        cache_file = self.cache_dir / f"{safe_step}_{prompt_hash}.json"
        ...
        response = self._client.call_json(step, model, prompt)
